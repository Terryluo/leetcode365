...
public class PseudoPalindromicPaths_1457 {
    ...
    private void dfs(TreeNode root, Map<Integer, Integer> record, int[] result) {
        // base case
        if (root == null) {
            return;
        }
        ...
        dfs(root.left, record, result);
        dfs(root.right, record, result);
        record.put(root.val, record.getOrDefault(root.val, 0) - 1);
    }

    public static void main(String[] args) {
        PseudoPalindromicPaths_1457 ppp = new PseudoPalindromicPaths_1457();
        TreeNode root = TreeNode.reconstructTreeForLeetcode(Arrays.asList(2,3,1,3,1,null,1));
        int result = ppp.pseudoPalindromicPaths(root);
